...
function runHalmos(srcPath: string): RunResult {
  try {
    const result = child_process.execSync(
      `halmos "${srcPath}" --max-width 100 --max-depth 100 --solver-threads 16`,
...
function runFoundryTest(
  contractDir: string,
  fuzzRuns: number = 1000,
): RunResult {
  try {
    const result = child_process.execSync(
...
function getContractSkeleton(dir: string): string {
...
async function main() {
  ...
  if (args.length < 3) {
    console.error(
      "Usage: ts-node src/workflow.ts <src_path> <uml_path> <invariant_path> [moddable_files...]",
    );
    console.error(
      "Example: ts-node src/workflow.ts ./test/test3/src ./test/test3/auction3.xmi ./test/test3/test/ file1.sol file2.sol",
    );
    process.exit(1);
  }
